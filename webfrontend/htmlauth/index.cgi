...
# ---------------------------------------
if (param('savedata')) {
	$conf = new Config::Simple("$home/config/plugins/$psubfolder/sepilox.cfg");
	...
	$conf->param('SENEC.USERNAME', unquotemeta($username));
	$conf->param('SENEC.PASSWORD', unquotemeta($password));
	$conf->param('SENEC.MINISERVER', unquotemeta("MINISERVER$miniserver"));
	$conf->param('SENEC.UDPPORT', unquotemeta($msudpport));
	$conf->param('SENEC.ENABLED', unquotemeta($enabled));
...
# ---------------------------------------
$conf = new Config::Simple("$home/config/plugins/$psubfolder/sepilox.cfg");
$username = encode_entities($conf->param('SENEC.USERNAME'));
$password = encode_entities($conf->param('SENEC.PASSWORD'));
$miniserver = encode_entities($conf->param('SENEC.MINISERVER'));
$msudpport = encode_entities($conf->param('SENEC.UDPPORT'));
$enabled = encode_entities($conf->param('SENEC.ENABLED'));
...
# Title
$template_title = $phrase->param("TXT0000") . ": SEPILOX";
